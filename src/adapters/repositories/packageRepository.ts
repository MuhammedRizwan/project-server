...
export class MongoPackageRepository {
  async createPackage(packages: Package): Promise<Package | null> {
    const packageDoc = await packageModel.create(packages);
  ...
  async editPackage(id: string, packageData: PackageData): Promise<Package | null> {
